...
for(i in 1:n_sims){
  ...
  incid_est1_par_ci_cov[i] = as.integer(log(m1_lambda_est) - qnorm(0.975)*sd(log(incid_est1_bs_par_vec)) <= log(true_incid) &
                                          log(m1_lambda_est) + qnorm(0.975)*sd(log(incid_est1_bs_par_vec)) >= log(true_incid) & m1_lambda_est > 0 & (!is.na(sd(log(incid_est1_bs_par_vec)))))
  
  incid_est2_par_ci_cov[i] = as.integer(log(m3_lambda_est) - qnorm(0.975)*sd(log(incid_est2_bs_par_vec)) <= log(true_incid) &
                                          log(m3_lambda_est) + qnorm(0.975)*sd(log(incid_est2_bs_par_vec)) >= log(true_incid) & m3_lambda_est > 0 & (!is.na(sd(log(incid_est2_bs_par_vec)))))
}
